...
.gb_meta <- function(
  ...
  if (grepl("^[0-5]$", adm_lvl)) adm_lvl <- paste0("ADM", adm_lvl)
...
get_adm_shp_link <- function(
  ...
  cache_dir <- rgeoboundaries_cache_dir()
  ...
  if (!all(file.exists(destfiles)) | isTRUE(force)) {
    ...
    lapply(
      seq_along(urls),
      \(i) download.file(urls[i], destfile = destfiles[i], quiet = quiet)
    )
...
get_cgaz_shp_link <- function(adm_lvl = "adm0", quiet = TRUE, force = FALSE) {
  ...
  if (grepl("^[0-2]$", adm_lvl)) adm_lvl <- paste0("adm", adm_lvl)
